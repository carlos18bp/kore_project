...
> **Kore Project override**: this repo uses a **mix of FBV and CBV** — both are valid here. Auth, profile, password-reset, and captcha endpoints are FBV with `@api_view` (~18 functions). Resource CRUD (Package, Subscription, Booking, Payment, Notification, FAQ, etc.) uses **DRF ViewSets** (~10+ classes). Some custom list/create endpoints use **APIView** (~6+). Match the style of the file you are touching; do not enforce a single pattern.
...
2. **Run only modified test files**: `pytest core_app/tests/path/to/test_file.py -v`. `pytest.ini` already sets `DJANGO_SETTINGS_MODULE=core_project.settings`.
