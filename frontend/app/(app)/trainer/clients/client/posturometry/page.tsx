            ...
            <div key={sf.key} className="flex items-center gap-3">
              ...
              {sf.type === 'severity' ? (
                <SeveritySelect
                  ...
                  onChange={(v) => onSubFieldChange(sf.key, v)}
                />
              ) : (
                <label className="flex items-center gap-1.5 cursor-pointer">
                  <input
                    ...
                    onChange={(e) => onSubFieldChange(sf.key, e.target.checked)}
        ...
        <div className="px-5 pb-5">
            ...
            {segments.map((def) => (
              <SegmentRow
                ...
                onSubFieldChange={(subKey, value) => {
                  const current = getEntry(def.key);
                  const newSubFields = { ...current.sub_fields, [subKey]: value };
                  const sevValues = Object.values(newSubFields)
                    .filter((sv): sv is string => typeof sv === 'string' && sv !== '')
                    .map((sv) => sv === 'L' ? 1 : sv === 'M' ? 2 : sv === 'S' ? 3 : 0);
                  let newSeverity = current.severity;
                  if (sevValues.length > 0) {
                    newSeverity = Math.max(...sevValues);
                  } else if (typeof value === 'boolean' && value && current.severity < 1) {
                    newSeverity = 1;
                  }
                  updateEntry(def.key, { sub_fields: newSubFields, severity: newSeverity });
                }}
              />
            ))}
          </div>
          ...
          <div className="mt-4 space-y-3">
            <div>
              ...
              />
              {compressing && <p className="text-xs text-amber-600 mt-1">Optimizando imagen...</p>}
              {photo && !compressing && (
                <p className="text-xs text-green-600 mt-1">
                  {photo.name}{sizeInfo && <span className="text-kore-gray-dark/40 ml-2">({sizeInfo})</span>}
                </p>
              )}
            </div>
            <div>
...
function TrainerPosturometryPage() {
  ...
  const { evaluations, loading, submitting, error, fetchEvaluations, createEvaluation, updateEvaluation, fullUpdateEvaluation, deleteEvaluation } = usePosturometryStore();
  ...
  const handleEdit = (ev: PosturometryEvaluation) => {
    setForm({
      evaluation_date: ev.evaluation_date || '',
      anterior_data: ev.anterior_data || {},
      lateral_right_data: ev.lateral_right_data || {},
      lateral_left_data: ev.lateral_left_data || {},
      posterior_data: ev.posterior_data || {},
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mb-6">
            ...
            <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-6 border border-white/60 shadow-sm">
              ...
              <div className="flex gap-3">
                ...
                <button type="button" onClick={() => { setShowForm(false); setEditingId(null); setForm(emptyFormData()); }} className="bg-kore-cream hover:bg-kore-gray-light/60 text-kore-gray-dark font-medium py-2.5 px-6 rounded-xl transition-colors text-sm cursor-pointer">
                  Cancelar
                </button>
              </div>
            </div>
