...
function PillarCard({ pillar }: { pillar: typeof pillars[number] }) {
  return (
    <div className="group flex flex-col items-center text-center">
      <div className="relative w-32 h-32 md:w-48 md:h-48 lg:w-56 lg:h-56 mb-4 md:mb-8 rounded-full overflow-hidden bg-kore-cream">
        <Image
          src={pillar.image}
          alt={pillar.alt}
          fill
      ...
      <h3 className="text-lg md:text-xl lg:text-2xl mb-2 md:mb-3">{pillar.title}</h3>
      <p className="text-sm md:text-base text-kore-gray-dark/70 leading-relaxed max-w-sm">
        {pillar.description}
      </p>
    </div>
  );
    ...
    <section ref={sectionRef} className="bg-kore-cream py-10 lg:py-12 overflow-hidden">
      <div className="w-full px-5 md:px-10 lg:px-16">
        ...
        <div className="text-center max-w-3xl mx-auto mb-8 md:mb-12">
          ...
          <h2 data-animate="split-text" data-delay="0.1" className="text-xl sm:text-2xl md:text-4xl lg:text-5xl mb-8 md:mb-8">
            KÓRE representa el origen, el núcleo, el centro desde donde el cuerpo se organiza y se adapta.
          </h2>
          <p data-animate="fade-up" data-delay="0.2" className="text-sm md:text-lg text-kore-gray-dark/70 leading-relaxed mt-2">
            Es un enfoque donde el entrenamiento deja de ser una rutina para convertirse en un proceso de salud guiado.
          </p>
        </div>

        {/* Pillars - Swiper on mobile, grid on md+ */}
        <MobileSwiper slidesPerView={1.15} spaceBetween={16} autoplayDelay={4000}>
