...
export default function LoginClient() {
  ...
  const getPostLoginRedirect = (role: string) => {
    ...
    if (redirect) return token ? `${redirect}?token=${token}` : redirect;
    return role === 'trainer' ? '/trainer/dashboard' : '/dashboard';
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      router.push(getPostLoginRedirect(currentUser?.role ?? 'customer'));
    } else {
      ...
      <div className="w-full max-w-md mx-auto px-6 pt-24 pb-16 md:py-16">
        ...
        <div data-hero="body" className="bg-white/60 backdrop-blur-sm rounded-2xl p-8 shadow-sm border border-kore-gray-light/50">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label htmlFor="email" className="block text-xs font-medium tracking-widest uppercase text-kore-gray-dark/40 mb-2">
                Correo electrónico
              </label>
              <input
                id="email"
            ...
            <div>
              <label htmlFor="password" className="block text-xs font-medium tracking-widest uppercase text-kore-gray-dark/40 mb-2">
                Contraseña
              </label>
            ...
            <div className="flex justify-end">
              <Link href="/forgot-password" className="text-xs text-kore-gray-dark/40 hover:text-kore-red transition-colors">
