...
Gating ($output-protocol §4): con `$ARGUMENTS` o intención clara en la sesión (la vista/componente a diseñar) → ejecutar directo, PROHIBIDO preguntar el tema. Sin argumentos ni contexto → UNA sola pregunta corta en texto por la vista o el componente (no picker: el insumo es libre). Nunca en modo fleet/headless.
